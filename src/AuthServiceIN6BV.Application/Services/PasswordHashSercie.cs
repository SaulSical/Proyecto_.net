...
public class PasswordHashService : IPasswordHashService
{
    ...
    public bool VerifyPassword(string password, string hashedPassword)
    {
        try
        {
            ...
            {
                Console.WriteLine("[DEBUG] Using Argon2id standard for verification.");
    ...
    private bool VerifyArgon2StandardFormat(string password, string hashedPassword)
    {
        try
        {
            var argon2Verfier = new Argon2id(Encoding.UTF8.GetBytes(password));
            ...
            argon2Verfier.Salt = salt;
            argon2Verfier.Iterations = iterations;
            argon2Verfier.MemorySize = memory;
            argon2Verfier.DegreeOfParallelism = parallelism;
 
            var computedHash = argon2Verfier.GetBytes(expectedHash.Length);
 
            return computedHash.SequenceEqual(computedHash);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[DEBUG] Error verifying Argon2 standard format: {ex.Message}");
            return false;
        }
    }
