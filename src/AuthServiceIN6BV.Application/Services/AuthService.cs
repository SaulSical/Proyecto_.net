    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        {
            logger.LogRegistrationWithExistinEmail();
            throw new BusinnessException(ErrorCode.EMAIL_ALREADY_EXISTS, "Email already exists");
        }

        // Verificar si el username ya existe
        if (await userRepository.ExitsByUsernameAsync(registerDto.UserName))
        {
            logger.LogRegistrationWithExistinUsername();
            throw new BusinnessException(ErrorCode.USERNAME_ALREADY_EXISTS, "Username already exists");
        }
        ...
        if (registerDto.ProfilePicture != null && registerDto.ProfilePicture.Size > 0)
        {
            ...
            if (!isValid)
            {
                ...
                throw new BusinnessException(ErrorCode.INVALID_FILE_FORMAT, errorMessage!);
            }
            ...
            catch (Exception)
            {
                ...
                throw new BusinnessException(ErrorCode.IMAGE_UPLOAD_FAILED, "Failed to upload profile image");
            }
        }
        else
        {
            profilePicturePath = _cloudinaryService.GetDefaulAvatarUrl();
        }
        ...
        var defaultRole = await roleRepository.GetByNameAsync(RoleConstans.USER_ROLE);
        if (defaultRole == null)
        {
            throw new InvalidOperationException($"Default role '{RoleConstans.USER_ROLE}' not found. Ensure seeding runs before registration.");
        }
    ...
    {
        var userRole = user.UserRoles.FirstOrDefault()?.Role?.Name ?? RoleConstans.USER_ROLE;
    ...
    private UserDetailsDto MapToUserDetailsDto(User user)
    {
        return new UserDetailsDto
        {
            ...
            Username = user.UserName,
            ProfilePicture = _cloudinaryService.GetFullimageUrl(user.UserProfile?.ProfilePicture ?? string.Empty),
            Role = user.UserRoles.FirstOrDefault()?.Role?.Name ?? RoleConstans.USER_ROLE
        };
    }
